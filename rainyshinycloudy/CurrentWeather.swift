            ...
            if let dict = result.value as? Dictionary<String, AnyObject> {
                ...
                if let main = dict["main"] as? Dictionary<String, AnyObject> {
                    if let currentTemperature = main["temp"] as? Double {
                        let kelvin2FarenheitPreDivision = (currentTemperature * (9/5) - 459.57)
                        let kelvin2Farenheit = Double(floor(round(kelvin2FarenheitPreDivision*10))/10)
                        //kelvin2Farenheit = Double(kelvin2Farenheit).roundTo(places: 1)
                        self._currentTemp = kelvin2Farenheit
